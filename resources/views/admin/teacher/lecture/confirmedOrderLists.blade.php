            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <br>
                        <div class="col-md-12">
                            <button class="btn btn-success order-checkin" data-lecture-order-id="{{ $list->lecture_order_id }}" data-teacher-id="{{ $teacher_id }}">簽到</button>
...
@section('javascript')
    <script>
        $( document ).ready(function() {
            $(document).on('click', '.order-checkin', function() {

                var lecture_order_id = $(this).data('lecture-order-id');
                var teacher_id = $(this).data('teacher-id');

                var checkinByAjaxURL = "<?php echo route('tch.admin.orderCheckinByAjax')?>";

                if(confirm('確定簽到?')){
                    $.ajax({
                        type: 'POST',
                        url: checkinByAjaxURL,
                        data: {
